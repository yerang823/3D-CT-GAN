...
config['unhealthy_scans_raw'] = "data/dcm_241/" #path to directory where the unhealthy scans are
config['unhealthy_coords'] = "data/NoduleList_10to50_severalRemoved_285.csv" #path to csv where each row indicates where a healthy sample is (format: filename, x, y ,z)
config['unhealthy_samples'] = "data/seed/unhealthy_samples_75844_32x32x32.npy" #path to pickle dump of processed healthy samples for training.
...
config['modelpath_inject'] = "result/model" #path to save/load trained models and normalization parameters for injector
...
config['progress'] = "result/img_progress" #path to save snapshots of training progress
...
#config['cube_shape'] = np.array([16,16,16]) #z,y,x
#config['mask_xlims'] = np.array([4,12])
#config['mask_ylims'] = np.array([4,12])
#config['mask_zlims'] = np.array([4,12])

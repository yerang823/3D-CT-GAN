...
def load_dicom(path2scan_dir):
    ...
    dcms = glob(dicom_folder+'/*.dcm')
    print(len(dcms))
    #first_slice_data = pydicom.read_file(path2scan_dir + '/' + dcms[0])
    first_slice_data = pydicom.read_file(dcms[0])
    first_slice = first_slice_data.pixel_array
    orientation = np.transpose(first_slice_data.ImageOrientationPatient) #zyx format (6,)
    spacing_xy = np.array(first_slice_data.PixelSpacing, dtype=float)
    spacing_z = np.float(first_slice_data.SliceThickness)
    spacing = np.array([spacing_z, spacing_xy[1], spacing_xy[0]]) #zyx format (1,3) 
    ...
    for dcm in dcms:
        if dcm.split('.')[-1] == 'dcm':
            ...
            slice_data = pydicom.read_file(dcm)
    ...
    for i, slice in enumerate(raw_slices):
        ...
        scan[i, :, :] = (slice.RescaleSlope * slice.pixel_array) + slice.RescaleIntercept
...
def save_dicom(scan, origional_raw_slices, dst_directory): # \\dfds\\ format
    ...
    for i, slice in enumerate(origional_raw_slices):
        ...
        ori = (scan[i,:,:]-slice.RescaleIntercept) / slice.RescaleSlope
        slice.pixel_array.flat = ori.flat
...
def world2vox(world_coord, spacing, orientation, origin):
    ...
    #print("np.dot(orientation[:3], np.diag(spacing)).shape=",np.dot(orientation, np.diag(spacing)).shape)
    #print("world_coord - origin shape=",(world_coord - origin).shape)
    #print("np.dot(orientation[:3], np.diag(spacing))=",np.dot(orientation, np.diag(spacing)))
    #print("np.linalg.inv(np.dot(orientation[:3], np.diag(spacing)))=",np.linalg.inv(np.dot(orientation[:3], np.diag(spacing))))
    #print("world_coord - origin=",world_coord - origin)
    #world_coord = np.dot(np.linalg.inv(np.dot(orientation, np.diag(spacing))), world_coord - origin)
